...
const Navbar = ({ isLogin, wishes }) => {
  ...
  return (
    <>
      {!isAdmin && (<div className={`w-[95%] flex justify-center  m-auto  ${fixTop ? "fixed top-0  w-full   z-50 shadow " : ''}`}>
        <div className={`lg:w-[95%] xl:w-[80%] w-full sm:w-[80%] m-auto sm:rounded-b-[40px] z-10 absolute ${fixTop ? "h-[70px] sm:h-[80px]" : "h-[70px] sm:h-[100px]"} flex justify-between items-center bg-white sm:bg-black transition-all duration-300`}>
          ...
          <nav className="nav font-semibold text-lg">
            <ul className={`items-center flex flex-col bg-[#85b130] lg:bg-black ئیjustify-between fixed top-0 bottom-0 ${isMenuOpen ? 'right-0' : 'right-[-300px]'} transition-all duration-300 pt-[5%] w-[300px] lg:pt-0 lg:static lg:flex lg:flex-row lg:w-full lg:mt-0 lg:mr-0`}>


              <button
                onClick={() => setIsMenuOpen(false)}
                className={`fixed right-[260px] top-[12px] lg:hidden cursor-pointer transition-all duration-300 
               ${isMenuOpen ? 'opacity-100  pointer-events-auto' : 'opacity-0 transition-all duration-100 pointer-events-none'}`}
              >
